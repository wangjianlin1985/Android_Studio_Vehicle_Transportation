...
public class YunshuEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.yunshu_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑运输单信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_yshw.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "运输货物输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_zl.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "重量(吨)输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_xysj.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "需要时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_qsd.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "起始地输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_mudidi.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "目的地输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_gonglishu.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "公里数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_yunshuMemo.getText().toString().equals("")) {
						Toast.makeText(YunshuEditActivity.this, "运输备注输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传运输单信息*/
					YunshuEditActivity.this.setTitle("正在更新运输单信息，稍等...");
					String result = yunshuService.UpdateYunshu(yunshu);
	...
	private void initViewData() {
		...
		this.TV_yunshuId.setText(yunshuId+"");
		for (int i = 0; i < userInfoList.size(); i++) {
			if (yunshu.getUserObj().equals(userInfoList.get(i).getJiahao())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < carList.size(); i++) {
			if (yunshu.getCarObj() == carList.get(i).getCarId()) {
				this.spinner_carObj.setSelection(i);
				break;
			}
		}
		this.ET_yshw.setText(yunshu.getYshw());
		this.ET_zl.setText(yunshu.getZl());
		this.ET_xysj.setText(yunshu.getXysj());
		this.ET_qsd.setText(yunshu.getQsd());
		this.ET_mudidi.setText(yunshu.getMudidi());
		this.ET_gonglishu.setText(yunshu.getGonglishu());
		this.ET_yunshuMemo.setText(yunshu.getYunshuMemo());
	}
	@Override
