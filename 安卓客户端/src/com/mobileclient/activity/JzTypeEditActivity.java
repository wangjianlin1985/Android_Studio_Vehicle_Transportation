...
public class JzTypeEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.jztype_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑驾照类型信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_typeName.getText().toString().equals("")) {
						Toast.makeText(JzTypeEditActivity.this, "类型名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传驾照类型信息*/
					JzTypeEditActivity.this.setTitle("正在更新驾照类型信息，稍等...");
					String result = jzTypeService.UpdateJzType(jzType);
	...
	private void initViewData() {
		...
		this.ET_typeName.setText(jzType.getTypeName());
	}
	@Override
